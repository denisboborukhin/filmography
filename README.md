...
watchlist, favorite actors and lead filmmakers, a movable taste map, local taste matches, and the
latest successful AI recommendation set.
...
ranked from your ratings, genres, tags, and TMDB metadata; their candidate selection is not generated
by AI. During `recommend`, the configured model can recalibrate their expected scores alongside
unscored watchlist entries. The updater does not crawl arbitrary review or streaming sites.

For watched titles, the updater also imports principal cast and one lead filmmaker from TMDB. It
uses the director when available and otherwise falls back to the most important producer credit.
The Taste map ranks recurring people using your personal scores and connects every enriched watched
title to recurring collaborators. Related titles form nearby islands in one shared map, while titles
without a person appearing in another visible film remain standalone with their own cast around
them. The map can filter by minimum personal score and principal cast size.
It can also limit the number of visible films, keeping the highest personal scores first.
This analysis is deterministic and does not use the AI provider.
...
fields. If a note has no review body, the site leaves the review empty rather than substituting
catalog text. The watchlist can be a simple plain-text list with one film title per line.
...
`--deterministic-limit`; `recommend` additionally accepts `--count` (5–20, default 10) and
`--prompt`.

`build` can import notes without a TMDB token, but catalog metadata, people credits, the Taste map,
and deterministic discoveries are available only when `TMDB_ACCESS_TOKEN` is set. Credit request
failures are reported as warnings and retain the previous successful credits when possible.
